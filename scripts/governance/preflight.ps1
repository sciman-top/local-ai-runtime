...
$records.Add((New-GateRecord -Gate 'hotspot' -Status 'gate_na' -Reason 'the v3.22 slice changes planning contracts rather than executable runtime hot paths' -AlternativeVerification 'planning governance tests + verifier + selector + git diff --check' -EvidenceLink 'docs/specs/acceptance-and-gates.md' -ExpiresAt 'the first executable runtime slice after LAR-P0D-001'))
