...
- 下一最小里程碑：队列 `LOCAL-AI-RUNTIME-0.2-BASELINE-CLOSURE` 的 `close_baseline_normative_package_first / LAR-P0A-008`。`LAR-P0A-007` 已闭合 `EvidenceContractSet.v1`、exhaustive event/status matrix、secret-safe append-only journal/receipt、no-replace artifact、activation-bound external evidence、purpose-separated key envelopes 与 anti-rollback backup/restore；规范包 `7/15 present, 8 missing`，最终 manifest 仍不存在。
