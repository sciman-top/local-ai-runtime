...
- 下一最小里程碑：执行 `LOCAL-AI-RUNTIME-0.2-BASELINE-CLOSURE` 的唯一 selectable 工作项 `LAR-P0A-003`；当前 selector 必须返回 `close_baseline_normative_package_first`。`LAR-P0A-002` 已完成 v3.23 manifest contract 重验，但最终 manifest 仍不存在。
