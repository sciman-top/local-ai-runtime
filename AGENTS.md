...
- `uv run --project .\runtime\host-orchestrator python -m host_orchestrator --repo-root . --revalidate-evidence-index <path-to-evidence_index.json>`: recompute task-level evidence digests without trusting the original writer.
...
## Testing Guidelines
Any change to the orchestrator mainline docs should run `python .\scripts\verify-planning-status.py`. Any change to repo-level governance routing should also run `python .\scripts\select-next-work.py` and `pwsh .\scripts\governance\preflight.ps1 -DisableAutoCommit`. Any change to `runtime/host-orchestrator` should run the relevant Python tests, and any change to canonical result/evidence handling should also run the `--revalidate-evidence-index` entrypoint against a generated or checked-in `.ai/runs/.../evidence_index.json`. Treat `build -> test -> contract/invariant -> hotspot` as the default order. New task/result examples should follow `T-YYYYMMDD-HHMMSS-slug.md`.
