...
- 下一最小里程碑：执行 `LOCAL-AI-RUNTIME-0.2-BASELINE-CLOSURE` 的唯一 selectable 工作项 `LAR-P0A-EVAL-002`；当前 selector 必须返回 `run_native_thin_path_evaluation_first`。
- Native thin-path / capability comparative evaluation contract 已冻结为三份 exact-byte JSON；下一步只可在固定 snapshot 的 disposable detached worktree 执行比较。CLI、App Server、SDK、managed Worktree、Automations 仍均未资格化；任一语义变更必须冻结 v3.23 并创建后继 candidate，不能原地改写。
