...
- 当前 repo-side 已把 `planner_required` 的 risk/dependency/force-on 触发接到 `waiting_handoff` handoff 路径，并把 worker-profile 不满足 `execution_lane / requires_network / requires_gui` 或超出 `max_active_leases` 的任务同样送去 handoff
