...
- `remote_non_gui` runner wiring readiness 已有 repo-side contract：`runner_wired=false` 时继续 pre-worker handoff，临时 `runner_wired=true` 测试配置必须绑定 repo-relative `runner_acceptance_ref` 后才会调用注入 runner，runner 异常会保持 failed dispatch 且不写成功 result；当前 committed profile 仍未落真实 remote runner
