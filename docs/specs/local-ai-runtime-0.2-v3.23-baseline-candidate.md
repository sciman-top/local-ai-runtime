# Local AI Runtime 0.2 v3.23：Windows-Local Governed AI Development Runtime Baseline Candidate

## 0. 结论与状态

- 本文的 narrative specification ID 为 <code>local-ai-runtime-0.2-v3.23</code>；它只与本文最终归档的精确字节一对一绑定。本文字节或本文声明的语义发生变化时必须创建新的 narrative specification ID，禁止同一 ID 对应不同正文。
...
- v3.23 语义上完整替代 v3.14-v3.22。实现者不得查阅旧正文或旧评审来补全缺失字段；v3.22 仅作为 exact-byte superseded candidate、历史规范包和任务图的可验证输入。
...
- <code>@openai/codex 0.144.1 + gpt-5.6-sol/high</code> 只是不改变语义的首个实验 profile；v3.23 要先在固定任务集上比较同一 effort 与一个较低 effort，才决定后续 profile 选择。它们是 generation 输入，不是不可演进的产品身份。
...
- 本文已声明但当前缺失的 artifact，首次按其声明的 artifact ID/version 物化，只是关闭 package inventory 中的 missing 项，不改变 v3.23 narrative bytes 或 ID；
...
### 16.2 Baseline Approval 前的 Native thin-path / capability comparative evaluation

在任何 Baseline Approval 前，必须先完成一个只读或 disposable-repo 的 <code>NativeThinPathCapabilityEvaluation.v1</code>。它不是 runtime 实现、Approval、Truth Reset、Batch claim、writer、remote push 或 live evidence；它只是决定 v3.23 的 capability contract 是否足够稳定、应当保持原义还是必须再 supersede。

评测必须冻结同一 repo snapshot、任务族/难度分层、任务输入与 success oracle、模型/effort、允许工具 inventory、sandbox、GateGraph、人工介入计时定义、重复次数、stop rule 和 evidence schema。至少比较：

1. 精简 Codex Native：短 AGENTS guidance + task-local repo skill + 单 agent Native Direct/Spec；
2. Native + 关键 gates：同一 Native 路径，但使用现有 test/contract/review/evidence closeout；
3. 只在任务形状适配时的 Superpowers、Trellis 或 Hermes 对照：它们不得成为第二 control plane，且不适配场景记 <code>not_applicable</code>，不强行凑对比；
4. CLI、App Server、SDK、managed Worktree、Automations 各自独立的 capability probe，不得把一个 surface 的成功外推到另一个。

<code>grill-me</code> 与 <code>grill-with-docs</code> 不是上述执行器的替代项：它们只可在不可逆、需求高不确定的人工决策阶段作为可选澄清输入；逐题人机问答、CONTEXT/ADR 草稿或第三方 skill 的声明都不得自动成为本仓事实、批准或 Batch admission。

每个 variant 至少记录 task success、漏检/回归、安全/gate/evidence 完整率、净人工分钟、P50/P95 wall time、token/成本、冲突/返工、recovery/rollback 成功率，以及抽样下游接受结果。效率只在 quality、safety 和 evidence 的硬下限均不下降时才有效；任一 hard floor 下降、出现未知/未归属 external effect、或 recovery/rollback 不能复现即停止，不得用平均 token 或 wall time 抵消。

评测结果只可产生以下三种 machine-readable decision：<code>preserve_v3_23_semantics</code>、<code>narrow_profile_or_adapter_candidate</code>、<code>supersede_required</code>。若结果改变 Batch 禁止面、adapter、authority、并发、Q0 trigger、quality promotion 或事实来源，必须冻结 v3.23、创建 successor v3.24 candidate、lineage、inventory、DAG、selector 和 verifier；不得把结果原地写回 v3.23。若不改变规范语义，记录 non-normative decision/evidence 并继续 P0A。
...
- v3.23 本文及全部规范性 schema/catalog/table/example/fixture/verifier 实际落盘；
...
- 本文与 v3.23-bound <code>BaselineLineage.v2</code>、历史 v3.22 package/plan 归档落盘后，preapproval inventory 应如实报告 15 required / 2 present / 13 missing，approval inactive、Truth Reset/implementation/P2 均未开始；Native thin-path / capability evaluation 先于 P0A schema closure，selector 的唯一 ready work item 应为 <code>LAR-P0A-EVAL-001 / run_native_thin_path_evaluation_first</code>；
...
本文的决策状态是 <code>candidate / Request changes until Native thin-path evaluation and normative package closure</code>。总体架构讨论已收口；下一步是先冻结并执行 <code>LAR-P0A-EVAL-001/-002</code> 的 Native thin-path / capability evaluation，只有结果保持本候选语义时才继续 P0A 的 schema/catalog/transition/example/fixture/verifier 多文件规范包和一致性评审。若结果要求改变 Batch 禁止面、adapter、authority、并发、Q0 trigger、quality promotion 或事实来源，必须冻结本候选并创建 v3.24；不得原地改写。只有受控 external operator action 创建的 active BaselineApprovalRecord 存在且未撤销，才允许执行 Truth Reset 和实现；只有 Implementation Acceptance、staged Full Q0、activation pointer CAS 和 immediate preflight 共同形成 ActiveRuntimeIdentity，才允许创建第一个 P2 writer task。
