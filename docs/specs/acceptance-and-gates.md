...
本合同投影 `local-ai-runtime-0.2-v3.24`。当前为 `baseline_candidate`，package=`8/15 present, 7 non-present`，`blocking_stage=baseline_approval`，唯一 task=`LAR-P0A-009`。v3.23 candidate/package/plan 是 superseded history；predecessor Native comparison 是 non-normative evidence，不是当前 gate、profile promotion 或 approval input。
