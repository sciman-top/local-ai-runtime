...
本合同投影 `local-ai-runtime-0.2-v3.25`。当前为 `baseline_candidate`，package=`10/15 present, 5 non-present`，`blocking_stage=baseline_approval`，唯一 task=`LAR-P0A-011`。`QualificationGateCatalog.v1` 已通过离线 component verifier，但这不等于 live Full Q0。v3.24 candidate/package/plan 是 superseded history；predecessor evidence 不是当前 gate、profile promotion 或 approval input。
...
- v3.25 narrative byte_count/SHA-256 与 stable entry target 精确一致；
- `BaselineLineage.v4` 绑定 v3.24 candidate/package/plan archives，carry-forward 恰为七项且 focused verifier green；
...
Baseline Approval 不要求 runtime code 存在，也不允许 AI 自签。v3.25 semantic change 必须先创建 v3.26 successor。
...
```json
{
  ...
  "current_work_item_id": "LAR-P0A-011"
}
```
...
`uv sync` 默认 exact，`--inexact` 明确禁止；`--exact` 不是可执行选项。`--locked` 只断言 lock 不变，`--offline` 只断网，`--no-python-downloads` 只禁止自动 Python 下载，`--no-sync` 不验证环境是否最新；任何单项都不能替代完整 toolchain identity read-back。
...
- Windows environment catalog key 使用 `OrdinalIgnoreCase` 唯一性，拒绝 aliases、hidden `=X:`、NUL，输出排序 UTF-16 double-NUL block。
- `pre_resume_parent_environment_proof` 每次 launch 绑定 canonical UTF-16 bytes、grammar、uniqueness、order、limits、terminator、digest 与 CreateProcess flags；它只证明 parent 输入，必须在 `ResumeThread` 前完成。
- `post_resume_q0_child_environment_observation` 由专用 no-write child 在 resume 后的第一个 application action 调用 `GetEnvironmentStringsW` 并报告 digest；必须等于 parent proof，否则 `platform_incompatible`。任何 child pre-resume self-observation claim 均拒绝。
...
- 52-task graph 的 P1 implementation 与 11 contract projections 双向闭合；
...
Full Q0 至少覆盖：Job/handle/stdio/EOF、named objects、`pre_resume_parent_environment_proof`、`post_resume_q0_child_environment_observation`、sandbox/secret/keyring、repo/path/Git config/object/ref、process/gate timeout、SQLite/journal recovery、evidence/backup restore、write accounting/emergency reserve、exact toolchain、first-run/four templates、legacy ownership、task-side network deny。Full Q0 green 与 P2 Admission 同 gate，只开放一个 pilot。
