...
本合同投影 `local-ai-runtime-0.2-v3.23`。当前为 baseline candidate；source 与 v3.23-bound lineage present，其他 13 项仍 missing，尚未达到 Baseline Approval。P0A closure 前还必须先通过 Native thin-path / capability comparative evaluation 的 `preserve_v3_23_semantics` 决定；v3.22 是精确 superseded input。
...
```json
{
  "next_action": "run_native_thin_path_evaluation_first",
  "current_work_item_id": "LAR-P0A-EVAL-002"
}
```
