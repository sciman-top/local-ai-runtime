...
本合同投影 `local-ai-runtime-0.2-v3.25`。当前为 `baseline_candidate`，package=`9/15 present, 6 non-present`，`blocking_stage=baseline_approval`，唯一 task=`LAR-P0A-010`。v3.24 candidate/package/plan 是 superseded history；predecessor evidence 不是当前 gate、profile promotion 或 approval input。
