# Local AI Runtime 0.2 v3.21：Unified Native + Batch Deterministic Minimum-Operator Implementation Baseline Candidate

## 0. 结论与状态

- 本文的 narrative specification ID 为 <code>local-ai-runtime-0.2-v3.21</code>；它只与本文最终归档的精确字节一对一绑定。本文字节或本文声明的语义发生变化时必须创建新的 narrative specification ID，禁止同一 ID 对应不同正文。
...
- v3.21 语义上完整替代 v3.14-v3.20。实现者不得查阅旧正文或旧评审来补全缺失字段。
...
- v3.18 至少出现两份不同正文却使用同一 ID，固定记为 <code>conflicted_candidate_id</code>，不得选一份静默当作 canonical。v3.21 批准前，必须分别归档每份 v3.18，分配不同 review evidence locator，并独立计算 hash。
...
- 本文已声明但当前缺失的 artifact，首次按其声明的 artifact ID/version 物化，只是关闭 package inventory 中的 missing 项，不改变 v3.21 narrative bytes 或 ID；
...
- <code>BaselineApprovalRecord.v1</code> 是不可变记录，绑定 narrative specification ID/hash、BaselineManifest ID/hash、全部 artifact ID/version/hash、<code>approval_review_head</code>、批准者 SID hash、UTC 微秒时间和 generation。Approval head 必须是 manifest 中 <code>package_review_head</code> 的可验证后继，并包含 manifest closure review；后续 review 继续 append，但不改既有 manifest/approval bytes。
...
- RepoProfile.evidence_mode 必须为 runtime_external_v1。External仅表示位于目标repo之外；root必须精确等于当前RuntimeActivationRecord绑定的 <code>%LOCALAPPDATA%\LocalAIRuntime\evidence</code> 受管root或其controller分配的repo/task descendant，不接受operator提供的任意绝对路径。
...
- v3.21 本文及全部规范性 schema/catalog/table/example/fixture/verifier 实际落盘；
