# Local AI Runtime 0.2 v3.24：Windows-Local Governed AI Development Runtime Baseline Candidate

## 0. 结论与状态

- 本文的 narrative specification ID 为 <code>local-ai-runtime-0.2-v3.24</code>；它只与本文最终归档的精确字节一对一绑定。本文字节或本文声明的语义发生变化时必须创建新的 narrative specification ID，禁止同一 ID 对应不同正文。
...
- v3.24 是由已冻结 v3.23 触发的窄幅 successor：官方 uv 语义复核证明旧门禁不能保证 exact environment、manifest-pinned Python 或 hash-pinned build backend；同时首发模板、首次成功路径和 operator action 体验缺少可执行产品契约。该 finding 使 v3.23 不再具备 approval eligibility，但不否定其 Windows-local、单操作者、单 SQLite authority、全局 capacity=1、CLI-first、deterministic commit-only 主体架构。
- v3.24 语义上完整替代 v3.14-v3.23。实现者不得查阅旧正文或旧评审来补全缺失字段；v3.23 只作为 exact-byte superseded candidate、历史规范包、历史任务图和显式 carry-forward artifact 的可验证输入。
...
- <code>@openai/codex 0.144.1 + gpt-5.6-sol/high</code> 只是首个 CLI capability/profile candidate。v3.23 的 Native thin-path 比较作为非规范 predecessor evidence 保留，未晋升任何 profile；v3.24 不把该结果外推到 SDK、App Server、managed Worktree、Automations 或新的 CLI/toolchain generation。它们是 generation 输入，不是不可演进的产品身份。
...
- 本文已声明但当前缺失的 artifact，首次按其声明的 artifact ID/version 物化，只是关闭 package inventory 中的 missing 项，不改变 v3.24 narrative bytes 或 ID；
...
- Successor carry-forward 不是隐式继承。只有 <code>BaselineLineage.v3.compatible_predecessor_artifacts</code> 中逐项绑定 predecessor baseline、artifact ID/version/path/bytes/hash、兼容范围和重新验证命令的 artifact 才可继续标记 present；任何未列入、bytes 漂移、兼容范围无法机械证明或受 v3.24 新语义影响的 artifact 一律回到 missing 并创建新版本。v3.24 只 carry-forward <code>CanonicalizationPolicy.v1</code>、<code>ExecutionSafetyContractSet.v1</code>、<code>EvidenceContractSet.v1</code> 和 <code>DeterministicGitContractSet.v1</code>；产品/首发体验与 qualification/toolchain 相关契约必须分别创建 <code>ProductContract.v2</code> 和 <code>QualificationContractSet.v2</code>。
...
- Exact environment 的建立必须显式使用 <code>uv sync --exact --locked --offline --python &lt;manifest-python&gt; --no-python-downloads</code>；随后所有 gate 使用 <code>uv run --no-sync --offline --python &lt;manifest-python&gt; --no-python-downloads</code>。<code>--locked</code> 单独不等于 exact environment，禁止把保留 extraneous package 的 inexact sync 作为 acceptance evidence。
...
### 16.2 v3.23 successor provenance 与聚焦重验

v3.23 的 <code>NativeThinPathCapabilityEvaluation.v1</code> 已结束，只作为 non-normative predecessor evidence 保留。它证明当时 CLI generation 下 evaluator-owned gates/evidence/recovery 不能被 agent-side prompt gate 替代，但没有晋升 profile，也不证明 v3.24 的 uv/Python/build envelope。v3.24 不重复无关的 18 个 trial；只有产品路由、Native/Batch 边界、model/effort promotion claim 或 execution surface 发生变化时才重跑对应 evaluation cohort。
...
后续如果改变 Batch 禁止面、adapter、authority、并发、Q0 trigger、quality promotion、事实来源或首发 template 的 effect envelope，必须冻结 v3.24、创建 v3.25 candidate、lineage、inventory、DAG、selector 和 verifier；不得原地改写 v3.24。

### 16.3 P0A：规范包闭包

- 保留 v3.14-v3.23 精确谱系，归档 v3.23 preapproval inventory/work-item plan；由 <code>BaselineLineage.v3</code> 逐项验证四个不受 successor 语义影响的 carried artifacts。Product/Qualification 必须新建 v2，其他缺失 artifact 按既有 15 项 inventory 闭包。
...
& $uv sync --exact --locked --offline --no-python-downloads --python $python --project ./runtime/local-ai-runtime
...
- v3.24 本文及全部规范性 schema/catalog/table/example/fixture/verifier 实际落盘；
...
- v3.17 与两份冲突 v3.18 源字节已经归档并双路径验证；v3.21、v3.22、v3.23 均保持精确 superseded candidate identity，v3.23 package/plan 另存为 non-authoritative frozen history；
- 本文与 v3.24-bound <code>BaselineLineage.v3</code> 落盘后，preapproval inventory 应如实报告 15 required / 6 present / 9 non-present：source、lineage 与四个逐项 carry-forward artifact present，ProductContract.v2、QualificationContractSet.v2 及后续 state/Q0/migration/examples/verifier/manifest/review 尚未闭合；approval inactive、Truth Reset/implementation/P2 均未开始；selector 的唯一 ready work item应为 <code>LAR-P0A-004 / close_baseline_normative_package_first</code>；
...
本文的决策状态是 <code>candidate / Request changes until v3.24 normative package closure</code>。总体产品与架构方向保持收口；当前下一项是物化 <code>ProductContract.v2</code>，把首发旅程、四个 launch template、CLI/operator presentation 和产品指标变成 schema/catalog/examples/negative fixtures，再物化 <code>QualificationContractSet.v2</code> 与 exact-toolchain gate contract。若后续发现需要改变 Batch 禁止面、adapter、authority、并发、Q0 trigger、quality promotion、事实来源或 launch template effect envelope，必须冻结本候选并创建 v3.25；不得原地改写。只有受控 external operator action 创建的 active BaselineApprovalRecord 存在且未撤销，才允许执行 Truth Reset 和实现；只有 Implementation Acceptance、staged Full Q0、activation pointer CAS 和 immediate preflight 共同形成 ActiveRuntimeIdentity，才允许创建第一个 P2 writer task。
