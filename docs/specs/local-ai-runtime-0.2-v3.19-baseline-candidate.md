# Local AI Runtime 0.2 v3.19：Unified Native + Batch Deterministic Minimum-Operator Implementation Baseline Candidate

## 0. 结论与状态

- 唯一规范 ID 为 <code>local-ai-runtime-0.2-v3.19</code>。
...
- v3.19 语义上完整替代 v3.14-v3.18。实现者不得查阅旧正文或旧评审来补全缺失字段。
...
- v3.18 至少出现两份不同正文却使用同一 ID，固定记为 <code>conflicted_candidate_id</code>，不得选一份静默当作 canonical。v3.19 批准前，必须分别归档每份 v3.18，分配不同 review evidence locator，并独立计算 hash。
...
- 禁止 CR、NUL、除 LF 之外的 C0 控制字符、行内 HTAB、行尾 SP/HTAB。Markdown 强制换行使用 <code>&lt;br&gt;</code>。
...
- Batch 禁止多 writer、subagent、task Approval、SDK、多 Provider、动态 fallback、dependency install/restore/setup、task-side network、remote Git、fetch/merge/push、target-ref 更新和 task-ref 删除。
...
- Truth Reset 前，仓库真值仍是 <code>runtime/host-orchestrator</code>、<code>.ai/state/control-plane.db</code> 和 <code>PHASE-1-VERTICAL-SLICE</code>。
...
codex-home/               immutable managed Codex config/instructions
...
每个 attempt 独占 <code>profile/xdg-config/codex-sqlite/tmp/cache/spool/gate-runs/object-store/worktree/empty</code>。不同 attempt 不得共享任何可写 HOME、SQLite、APPDATA、TEMP、cache、spool 或 gate output。

| Class | Roots | Writer/gate view | Lifecycle |
|---|---|---|---|
| immutable_shared | versions, policies, schemas, codex-home, environments | exact allowlisted read/execute only | offline staged, content-addressed, activation 后不就地修改或删除 |
...
- 0.2 只支持通过 no-replace、ReplaceFileW、FlushFileBuffers、identity、ACL、hardlink、reparse 和 power-loss probe 的本地固定 NTFS 卷。
...
- <code>RuntimeActivationRecord/RuntimeToolchainManifest/QualifiedEnvironmentBinding</code>；
- <code>OwnershipWirePolicy/NamedObjectPolicy/ResourceLimitPolicy/SchedulerPolicy/BackupPolicy</code>；
...
- process/recovery：<code>JobIdentity/WriterLaunchRecord/JournalSegmentManifest/NormalizedExecutionEvent/AttemptAuthorizationContinuation</code>；
...
<code>QualificationObservation.v1</code> 绑定本次 observation 的 base commit/tree 和 working-tree audit；可复用的 <code>QualificationSensitiveInputSet.v1</code> hash 不包含整个 base commit/tree。
...
Set envelope 绑定 repo identity、local_base_ref 名称、resolver catalog ID/hash、discovery-rule generation、negative-discovery result 和完整有序 entry union；明确排除普通源码、observation base commit/tree、task ID 和 submission ID。
...
Batch 只验证和挂接 binding，不 install/restore/bootstrap，不执行 setup script。缺失、retired 或漂移进入 needs_environment。Retire 只阻止新引用；0.2 不物理删除 immutable bundle。既有 attempt 只有在原 binding identity 完整且仍获 Authorization 时才可继续。
...
- 不显式设置会启用 plaintext TUI log 的 log_dir；immutable CODEX_HOME/log 不存在或只读，Q0 必须证明运行成功且无未知日志。
...
### 7.4 Instructions、skills 与 prompt evidence
...
- CODEX_HOME 指向 immutable managed config/instructions。
...
Writer 进入固定 attempt-level Job；每个 gate、每个 Git sandbox command 和 recovery helper 进入自己的 durable StageJob。两类均为 kill-on-close/no-breakaway，并按规定 deadline 终止完整进程树；StageJob 不得承载 writer。
...
- Uniqueness 至少覆盖 family fingerprint/root、root+generation、task+attempt_no、每 task generation 一个 writer execution commit、attempt+event_seq、segment sequence、fenced head/result、resubmission source/successor、artifact logical name、object plan/set、task ref、active Authorization map 和 global capacity lease。
...
- 排除 auth content、mutable Codex state、environment bundle content、worktree、ordinary quarantine 和 nonterminal spool。
...
GIT_DIR、GIT_WORK_TREE、GIT_COMMON_DIR、GIT_INDEX_FILE、GIT_OBJECT_DIRECTORY、GIT_ALTERNATE_OBJECT_DIRECTORIES、GIT_EXEC_PATH、GIT_TEMPLATE_DIR、GIT_NAMESPACE 只由 adapter 为单次 operation 设置。
...
- finalize_worktree_head 单独执行 <code>git update-ref --no-deref HEAD &lt;expected-commit&gt; &lt;expected-base&gt;</code> 并验证。禁止 reset --hard。
- Task ref 只在 common reachability、index finalize 和 HEAD finalize 全绿后执行 <code>update-ref &lt;exact-task-ref&gt; &lt;expected-commit&gt; &lt;zero-oid&gt;</code>。
...
- Claim 前可用磁盘必须至少为 <code>max(5 GiB, 2 * base_materialized_bytes + 2 * artifact_limit + 2 * environment_writable_budget)</code>；其中 attempt writable environment budget 固定为 1 GiB，模板只能下调。
- 运行中低于 1 GiB emergency reserve 时停止新 gate/object/artifact，安全终止活进程、seal 并进入 needs_environment/recovery_pending；不得靠删除当前或其他未终态 attempt 恢复空间。
...
- <code>batch retry</code> 只执行第 9.4 节的同-generation RetryEligibility；不满足时返回唯一建议 <code>batch resolve &lt;source_task_id&gt; --code create_resubmission_v1</code>，自身绝不创建 successor。
...
| 5 | Temporarily unavailable、needs_auth 或 qualification_suspended |
...
2. Codex config：strict-config、profile、<code>trust_level=untrusted</code>、project config/hooks/rules skip、AGENTS/skills discovery、feature catalog、model-visible tool inventory、ephemeral/history/logging disk diff。
3. Auth/provider：keyring-only login/refresh/generation、needs_auth 分类、Codex parent 最小 control-plane connectivity、secret-free evidence。
...
5. Process isolation：empty environment、attempt-local HOME/APPDATA/SQLite/TEMP/cache、safe DLL loading、no profile/login shell、command-line/overlay boundaries。
6. Windows objects：SDDL、mutex collision、Job limits、atomic suspended join、no-breakaway、kill-on-close、same-name zero-process fail-closed、PID creation-time validation。
7. Filesystem：fixed local NTFS、no-replace、ReplaceFileW、FlushFileBuffers、power-loss probes、reparse/junction/symlink/hardlink escape、ACL drift 和 disk reserve。
...
10. Git：config deny catalog、protected surface、controlled index、tree ordering、object framing、existing-object canonical verification、unique parent/header order、index/HEAD/task-ref order、deterministic repeated commit。
...
- active queue 从 <code>PHASE-1-VERTICAL-SLICE</code> 改为 <code>MINIMUM-OPERATOR-COMMIT-ONLY-TRANSITION</code>；
...
2. SQLite storage、migrations、family/resubmission、lease/fence/adoption/continuation。
3. Installer/activation/backup、toolchain/environment、ownership/legacy conformance。
4. Qualification/Authorization/Codex overlay/auth/permission/Q0 probes。
5. Process/Job/marker/stream/journal/artifact/recovery。
6. Gate runner、secret scan、deterministic Git object/finalize/ref/evidence/cleanup。
...
2. needs_environment/register/requalify/retire；
3. platform unavailable/suspended/incompatible；
...
- 规范字节：UTF-8/no-BOM/LF/NFC/terminal-LF/control/trailing-space；
- Canonical JSON：重复 key、integer bounds、object order、preserved arrays、set_semantics unique sort key、domain separation；
- Git/Windows path：raw Git spelling、NFC validation、ADS/device/reserved/case collision、final identity、reparse/hardlink；
...
- Submission family：root identity、ordinary replay、atomic successor、response loss、v1/v2 resolution-family uniqueness、historical-ref prohibition；
- Authorization：reusable scope、sensitive-set change invalidation、ordinary base change non-invalidation、expiry/revoke/checkpoint continuation；
...
3. DB launch intent、CREATE_NEW marker、spawned_suspended PID、writer_execution_committed、ResumeThread result、process exit；
...
6. gate intent/process/report/terminal result；
...
9. GitObjectPlan、object materialization/set seal、promotion/existing-object verify/reachability；
10. index replace、HEAD CAS、task-ref CAS、evidence publish、worktree removal；
...
- v3.19 本文及全部规范性 schema/catalog/table/example/fixture/verifier 实际落盘；
...
不支持 SHA-256 object format、reftable、network/removable/cloud-synced worktree、remote/fetch/push、historical detached base、automatic merge/ref deletion、multi-writer、subagent Batch、runtime dependency install、GUI/production/database/credential mutation 和 unattended Native。发现 unsupported condition 必须在 prepare/qualification 阶段退出 3 或 6，不能半程适配。
...
- active queue 仍是 <code>PHASE-1-VERTICAL-SLICE</code>；
- planning verifier 因缺少 <code>本地 AI 运行时</code> 和 <code>host_local &gt; remote_non_gui &gt; vm_gui</code> 两个 token 退出 1；
- selector 返回 <code>repair_gate_first</code>；
