# Local AI Runtime 0.2 v3.25：Windows-Local Governed AI Development Runtime Baseline Candidate

## 0. 结论与状态

- 本文的 narrative specification ID 为 <code>local-ai-runtime-0.2-v3.25</code>；它只与本文最终归档的精确字节一对一绑定。本文字节或本文声明的语义发生变化时必须创建新的 narrative specification ID，禁止同一 ID 对应不同正文。
...
- v3.25 是由已冻结 v3.24 触发的最小 successor：Microsoft 文档证明 <code>CREATE_SUSPENDED</code> 下 primary thread 在 <code>ResumeThread</code> 前不运行，而 <code>GetEnvironmentStringsW</code> 只能读取调用进程自身环境，因此 v3.24 要求的“child-observed environment block before ResumeThread”不可实现。v3.24 还写入本机与当前 uv help 均不存在的 <code>uv sync --exact</code> 参数；exact sync 实际是默认行为，<code>--inexact</code> 才会保留 extraneous packages。
- v3.25 语义上完整替代 v3.14-v3.24，但不扩大产品范围：Windows-local、单操作者、单 SQLite authority、全局 capacity=1、CLI-first、deterministic commit-only 主体架构及首发产品契约保持不变。实现者不得查阅旧正文或旧评审来补全缺失字段；v3.24 只作为 exact-byte superseded candidate、历史规范包、历史任务图和七个显式 carry-forward artifact 的可验证输入。
- 产品长期定位是：面向 Windows 本机、单操作者信任域、以受资格化开发 authority 为锚的通用受控 AI 开发执行平台，其用户可感知目标是<strong>低人工、可预测、可恢复的开发吞吐</strong>。Architecture Epoch 1 / Runtime 0.2 只实现 <code>qualified_git_repo_v1</code> authority、全局 capacity=1 和单一 SQLite 控制面；不预建跨平台、多租户、分布式或第二 agent runtime 空壳。
- 0.2 的初始已知执行 surface 是 <code>codex_cli_executor_v1/sha1_files_refs_v1/local_commit_task_ref_v1/task_egress_denied_v1</code>。CLI/SDK 是执行接口，App Server 是 client protocol，managed Worktree 是 workspace isolation，Automations 是 scheduling surface；它们是正交、互不推定的 capability candidates，而不是同一种 <code>CodexPlatformAdapter</code>。每个 surface 及其实际组合都必须经过本候选 preapproval probe、Implementation Acceptance 和对应 Q0 资格化；任何未资格化实例不得因名称、同一安装包或配置开关而被继承启用。
- <code>@openai/codex 0.144.1 + gpt-5.6-sol/high</code> 只是首个 CLI capability/profile candidate。v3.23 的 Native thin-path 比较作为非规范 predecessor evidence 保留，未晋升任何 profile；v3.25 不把该结果外推到 SDK、App Server、managed Worktree、Automations 或新的 CLI/toolchain generation。它们是 generation 输入，不是不可演进的产品身份。
- 目标采用词典序：安全、副作用正确性和证据完整性为硬门；其次最小化净人工分钟；再提高无人值守完成率；最后优化 Native 交互延迟、Batch 可预测周期和可归属成本。Epoch 1 的 Batch 不承诺高速并发。
...
- <code>superseded_candidate</code>：v3.22，178,330 bytes，SHA-256 为 <code>8338A9DCF4BBBB40CA28F4F2EC6DCA37587EE94FBFBBC6E3A0063C4DE379569C</code>。v3.22 的“Python Policy/Evidence Kernel + Unified Native + 单 writer Batch”方向被保留；其在 Baseline Approval 前冻结平台 surface、把 SDK 按名称列入 Batch 禁止面、以及把下游接受结果只当未来语义的不足由本候选修正。
- <code>superseded_candidate</code>：v3.23，188,325 bytes，SHA-256 为 <code>80562322EBC744EDA2A87A17C45F73A11982F4947C9D10E8628BB6F73EE9D5C6</code>。对应 preapproval inventory 为 12,439 bytes、SHA-256 <code>2771B750557C68002EB54C3681185395E7D2881632211A4BB0BD6DE75644D620</code>；machine work-item plan 为 220,533 bytes、SHA-256 <code>4B146D79C0F99A9621A8F6A743E8DDAB7E2EB6D19356D2152A1C7130C0ADEBFE</code>。三者保持精确归档，不接受原地修改。
- <code>superseded_candidate</code>：v3.24，199,728 bytes，SHA-256 为 <code>13EE366152D47EDEC151F30619CCD068A030B63FEBF2D899EE822D08D4DC4E2A</code>。对应 preapproval inventory 为 15,646 bytes、SHA-256 <code>144383F8704F366008E9CB514898E05F1FD7A45310D39CD64BDC546544247A9F</code>；machine work-item plan 为 187,913 bytes、SHA-256 <code>10D48982B7B45F2C8033F1BA571ACEBA51106484347A71EC436121607F2518DF</code>。三者保持精确归档，不接受原地修改。
...
- 本文已声明但当前缺失的 artifact，首次按其声明的 artifact ID/version 物化，只是关闭 package inventory 中的 missing 项，不改变 v3.25 narrative bytes 或 ID；
...
- Successor carry-forward 不是隐式继承。只有 <code>BaselineLineage.v4.compatible_predecessor_artifacts</code> 中逐项绑定 predecessor baseline、artifact ID/version/path/bytes/hash、兼容范围和重新验证命令的 artifact 才可继续标记 present；任何未列入、bytes 漂移、兼容范围无法机械证明或受 v3.25 新语义影响的 artifact 一律回到 missing 并创建新版本。v3.25 只 carry-forward v3.24 已闭合且与本次环境观察时点修正兼容的 <code>CanonicalizationPolicy.v1</code>、<code>ProductContract.v2</code>、<code>QualificationContractSet.v2</code>、<code>ExecutionSafetyContractSet.v1</code>、<code>EvidenceContractSet.v1</code>、<code>DeterministicGitContractSet.v1</code> 和 <code>StatePolicyCatalog.v1</code>；Q0 及后续 artifact 仍为 non-present。
...
0.2 是该终态的 Epoch 1 首个受治理 profile，包含四个入口。Native 和 Batch 的速度语义分开：Native 快路径追求更少等待、精简上下文和低交互延迟；Batch 的单 writer 追求低人工、可预测、可恢复的 commit-only 交付，不能被表述为高速并发系统。
...
3. Codex Native Program：仅用于至少两个独立写集合且集成顺序已固定的任务；可使用已资格化的 Native subagents/worktrees，但由人工集成，不继承 Batch receipt/fence。
...
- Batch 禁止多 writer、subagent、task Approval、多 Provider、动态 fallback、controller/gate dependency install/restore/bootstrap、task-side network、remote Git、fetch/merge/push、target-ref 更新和 task-ref 删除；writer 侧按 QualifiedEnvironment、permission、network、path、write-budget 与 delta policy机械约束，不宣称能语义识别所有名为 setup/install 的源码操作。
- CLI、App Server、SDK、managed Worktree 和 Automations 不是同义词，也不是天然可互换的 Batch 特权。<code>ExecutionInterfaceCapability</code>（CLI 或 SDK）、<code>ClientProtocolCapability</code>（App Server）、<code>WorkspaceIsolationCapability</code>（managed Worktree）和 <code>SchedulingCapability</code>（Automations）各自以独立 surface、version/help/schema/tool inventory、sandbox/permission behavior、state/effect boundary、recovery/rollback 和 evidence projection 资格化；某个组合只在它的每个组成能力和组合 effect protocol 均通过资格化后才可被引用。评测胜出只可提出候选 capability，不能直接给 Batch 授权。未资格化 SDK 仍不得进入 Batch；已资格化 SDK 也只可在它自己的封闭 execution-interface protocol、effect ledger 和 Q0 generation 中使用。
- Batch 唯一交付为 local deterministic commit 与 <code>refs/heads/codex/batch/&lt;task_uuid&gt;-a&lt;attempt_no&gt;</code>。人工负责最终检查和 merge/push。
- <code>batch_commit_only_v1</code> 的禁止项是 0.2 generation 的硬安全边界，不是暗示未来能力已存在。新模型、effort、工具版本、已证明 envelope 内的收窄资源参数和 gate 选择可由新 ExecutionProfile generation 表达；CLI/SDK 执行接口、App Server client protocol、managed Worktree isolation、Automations scheduling 任一 surface的新行为、tool inventory、sandbox、SDK/App Server contract、workspace authority、network/delivery 或外部 effect 都必须创建新的 capability generation，并有对应的 capability/schema/effect ledger/recovery matrix、Implementation Acceptance 与 Full Q0；多 writer、多操作者、多信任域或同一 effect domain 并行必须创建 successor architecture epoch。协议未实现并验收前一律 <code>unsupported</code>，不得以配置开关启用。
...
### 2.5 首发产品体验与 launch templates

0.2 的首发体验是 CLI-first、可脚本化且不依赖 GUI 的“从未配置到第一个可审查本地 commit”闭环。<code>ProductContract.v2</code> 必须内嵌 <code>FirstRunExperiencePolicy.v1</code>、<code>LaunchTemplateCatalog.v1</code> 和 <code>OperatorPresentationCatalog.v1</code>；它们属于既有 <code>P0A-PRODUCT</code> artifact，不新增第十六个 normative artifact 或第二套产品真源。

首发旅程固定为：

~~~text
runtime doctor --setup-readiness
-> runtime install/qualify --staged
-> runtime activate --dry-run
-> repo inspect/qualify --repo <path>
-> template list --eligible --repo <repo_id>
-> authorization prepare/activate --repo <repo_id> --template <template_id>
-> batch submit --repo <repo_id> --template <template_id> --params <public-json> --expected-base <oid>
-> batch status/watch <task_id>
-> batch inspect <task_id>
-> human review local commit and runtime-owned task ref
~~~

每一步同时提供稳定 JSON 和面向操作者的简洁文本投影；文本只由版本化 catalog 根据 reason code、state 和公开 locator 渲染，不拼接 raw model/tool output。所有 mutation 命令默认先显示或返回 machine-readable effect summary、authority、expected generation、回滚入口和是否需要交互 challenge；read-only 命令不得要求 approval。<code>--json</code> 输出字段稳定、unknown field fail-closed，human output 可演进但不得改变 exit code 或事实语义。

首发 <code>LaunchTemplateCatalog.v1</code> 只包含四个可独立 qualification/promotion 的 template family：

| Template ID | 首发用途 | 封闭参数 | 默认允许写面 | 必须 gates | 硬禁止 |
|---|---|---|---|---|---|
| <code>docs_contract_sync_v1</code> | 已知文档、README、契约投影同步 | approved document path IDs、source-of-truth ID、projection IDs | allowlisted Markdown/JSON/YAML docs 与 task evidence | format/parse、contract projection、link/path、diff budget | runtime code、dependency、secret、remote |
| <code>bounded_lint_type_repair_v1</code> | 修复已有 Ruff/Pyright/等价静态门的确定问题 | approved diagnostic IDs、allowlisted source path IDs、max files | 资格化源码 allowlist，默认最多 5 个 primary files | focused lint/type、related tests、full declared gate graph | 新依赖、公开 API 语义变更、配置降级、跳过规则 |
| <code>focused_test_repair_v1</code> | 修复一个已复现、已定位的 repo-local 测试失败 | test IDs、failure fingerprint、allowlisted source/test paths | 相关实现与测试 allowlist | original failure red evidence、focused green、declared regression/full gates | flaky-only 重跑冒充修复、删除/skip 测试、依赖安装、外部服务 |
| <code>mechanical_repo_maintenance_v1</code> | closed mapping 的 rename、生成物刷新或声明式整理 | versioned mapping、approved path set、generator ID、expected outputs | 映射/生成器声明的精确 path set | dry-run manifest、生成一致性、build/test/contract、unexpected delta=0 | 自由重构、动态发现新目标、remote/merge/push |

Template promotion 必须依次经过：candidate definition -> negative examples -> disposable dry-run -> repo/template qualification -> explicit operator promotion -> reusable Authorization。Native Spec 只可生成 candidate definition；不能自行 promotion。每个 template 的 schema 必须固定参数类型/上限、path/effect envelope、required/forbidden gates、risk class、expected outputs、stop reasons、recovery class、evaluation denominator 和 rollback。

首发状态体验至少提供：

- <code>runtime status</code>：active identity、platform lane、capacity、suspension/recovery reason 和下一合法 operator action；
- <code>repo status &lt;repo_id&gt;</code>：ownership、qualification、maintenance、base drift、active task/ref；
- <code>batch status/watch/inspect</code>：family/task/attempt state、当前 phase、公开 gate summary、commit/ref locator、cleanup disposition；
- <code>operator actions list/show/resolve</code>：scope、severity、stable reason code、plain-language explanation、recommended exact command、preconditions、expiry、evidence locator、是否阻塞其他 repo；
- <code>doctor --quick|--full</code>：只读、可重复、明确区分 configuration_error、needs_auth、needs_environment、platform_incompatible 和 transient unavailable。

产品验收新增以下指标，但不把目标值伪装成既有实测：prequalified host 到首个 dry-run 的净人工分钟、首个 commit-ready pilot 的净人工分钟、template qualification lead time、eligible-template coverage、operator-action age、policy false-block review rate、recovery-to-terminal time，以及每个成功任务的 template maintenance minutes。P4 cohort 前必须固定分母和采集方式；unknown 保留 unknown。
...
- <code>RuntimeToolchainManifest.v1</code> 锁定 Python 3.11.x 的具体 patch、uv、uv.lock、PowerShell、Git、Node/npm wrapper、<code>@openai/codex@0.144.1</code>、bundled codex.exe、Windows build、sandbox、Job helper、CodexPlatformCapabilitySet、bootstrap 和 Q0 probes 的 absolute path、version、SHA-256、volume/file identity 及必要 publisher。
...
- <code>VerificationExecutionProfile.v1</code> 是开发门禁、Implementation Acceptance 与 Full Q0 共用的唯一命令模板真源；work item 只引用 profile ID 和 focused verification，不复制整组 uv 命令。Profile 绑定 uv absolute path/version/hash、manifest-pinned Python absolute path/version/file identity/hash、<code>uv.lock</code>、hashed build-constraint file、project path、venv path、build backend identity、SOURCE_DATE_EPOCH policy 和每个 gate argv。任何投影命令与 profile 不一致均由 planning verifier 拒绝。
- Exact environment 的建立必须显式使用 <code>uv sync --locked --offline --python &lt;manifest-python&gt; --no-python-downloads</code>；<code>uv sync</code> 默认执行 exact sync，<code>--inexact</code> 明确禁止，且 <code>--no-build-isolation</code> 的 extraneous-package 例外不得进入本 profile。随后所有 gate 使用 <code>uv run --no-sync --offline --python &lt;manifest-python&gt; --no-python-downloads</code>。<code>--locked</code> 单独不等于 exact environment，<code>--no-sync</code> 也不验证环境是否最新；两者都必须依赖同 generation 的 preparation evidence 与 child identity read-back。
- Build 必须使用 manifest-bound、带 hash 的 build constraints 和 <code>--require-hashes</code>；<code>uv build --offline</code> 单独不能证明 build backend 身份。隔离 build 前后都 read-back backend distribution/version/hash；在两个全新 managed root 使用同一 SOURCE_DATE_EPOCH 重复构建并比较 normalized wheel/sdist member manifest 和最终 SHA-256，差异即阻断。
- 每次 sync/build/run 前后都验证 child <code>sys.executable</code>、Python patch、volume/file identity 和 SHA-256 与 RuntimeToolchainManifest 相同；错误 patch、PATH-only interpreter、uv-managed automatic download 或 venv interpreter 漂移一律 <code>needs_environment</code> 或 <code>platform_incompatible</code>，不得 fallback。
- Negative fixtures 至少覆盖 lock 外 package、自动发现的 pytest plugin、错误 Python patch、同 cache 多 build-backend version、缺失 build constraint/hash、uv download request 和 clean-root repeat-build mismatch。只有这些 fixture fail-closed，才能声称 exact/offline/reproducible gate envelope 已建立。
...
- <code>ExecutionInterfaceCapability/ClientProtocolCapability/WorkspaceIsolationCapability/SchedulingCapability/CodexPlatformCapabilitySet/Q0TriggerPolicy/Q0ProbeCatalog/ResolverCatalog/ReasonCodeCatalog</code>；
...
Provider 术语必须分开：<code>ModelProfile</code> 是 model/effort 参数；<code>ModelProviderCapability</code> 是 auth/control-plane/network/event contract；<code>AgentRuntimeCapability</code> 是 Codex execution/config/tool protocol；<code>ExecutionInterfaceCapability</code> 是 CLI 或 SDK 的明确执行接口 contract；<code>ClientProtocolCapability</code> 是 App Server contract；<code>WorkspaceIsolationCapability</code> 是 managed Worktree contract；<code>SchedulingCapability</code> 是 Automations contract。<code>CodexPlatformCapabilitySet</code> 只记录这些正交 surface 的资格化组合，不能把 worktree 或 automation 错称为 executable adapter。0.2 catalog 只允许经过资格化的实例；不得把第二 model 名称误当第二 Provider，也不得因同一 Codex 安装包而把一个 surface 的 Q0 结果外推到另一个 surface。

演进分为三个机械层级：

1. Profile generation：只在同一已资格化 <code>CodexPlatformCapabilitySet</code> 与 CapabilitySet 已证明 envelope 内选择 model/effort、timeout、resource limit、allow path 和 GateGraph；按 Q0TriggerPolicy 执行 scoped requalification、Authorization、canary 或 Full Q0。任何 model/effort、tool inventory、sandbox 或 agent-visible config 变化均不得作为普通热切换。
2. Capability generation：引入或改变 CLI/SDK execution interface、App Server client protocol、managed Worktree isolation、Automations scheduling、Provider、network、delivery、Git format、workspace authority、tool inventory、sandbox/permission behavior 或其他新外部 effect protocol；必须有对应 capability/schema/effect/recovery/migration、Implementation Acceptance 和 Full Q0，不能只改 profile。
...
运行态只有一个 RuntimeActivationBundle 和一个 ActiveRuntimeIdentity。Profile 只能引用已经出现在同一 C/I/Q 中的 capability；未实现或未共同验收的组合即使 schema 可表达也必须拒绝。每个 capability surface 的 qualification record 必须绑定 role、surface ID、官方 reference locator/date、本机 version/help/schema hash、tool inventory、sandbox/permission probe、允许的 state/effect envelope、recovery/rollback result 和 evidence projection；组合 record 还必须绑定组成 surface generation 与组合 effect protocol。一旦这些任一输入漂移，旧 capability generation 不得继续 claim。
...
<code>ProcessEnvironmentPolicy.v1</code> 还必须固定 Windows environment block 的字节级构造：key 使用 ASCII catalog spelling并在 <code>OrdinalIgnoreCase</code> map 中唯一；任意大小写 alias、重复 key、空 key、含 <code>=</code>/NUL 的 key或含NUL的value均拒绝。Windows drive-current-directory pseudo variables（例如 <code>=C:</code>）以及其他以 <code>=</code> 开头的hidden entry一律不生成、不继承；这是产品的窄化输入策略，不伪称为普通环境变量 API 对 pseudo entry 的完整规范。序列化只使用catalog中的canonical key spelling，按Windows case-insensitive ordinal comparator排序为 <code>key=value\0</code>，最后恰好再写一个NUL；总UTF-16 code units、单value长度和CreateProcess总命令行/environment边界均由ResourceLimitPolicy给出 <code>limit-1/limit/limit+1</code> fixture。ResourceLimitPolicy 的 aggregate ceiling 是本产品资源策略，不得表述为现代 Windows 的 OS environment-block total limit。

环境证明固定分为两个不可混同的阶段：

1. <code>pre_resume_parent_environment_proof</code>：parent 在调用 <code>CreateProcessW</code> 前或 child 仍 suspended 时，对将传入的 canonical UTF-16 block 验证 key grammar、OrdinalIgnoreCase uniqueness、排序、单值/总长度、恰好 double-NUL terminator、完整 digest，以及 <code>CREATE_UNICODE_ENVIRONMENT | CREATE_SUSPENDED | EXTENDED_STARTUPINFO_PRESENT</code> flags；任一 inherited block、case-fold collision、serialization 或 flag mismatch 都在 <code>ResumeThread</code> 前终止 suspended Job。
2. <code>post_resume_q0_child_environment_observation</code>：专用 no-write Q0 child 在 resume 后的第一个 application action 调用 <code>GetEnvironmentStringsW</code> 读取自身环境，按同一 policy canonicalize 并通过已资格化 stdout 报告 digest；该 digest 必须与 persisted pre-resume digest 相等，否则为 <code>platform_incompatible</code>。<code>CREATE_SUSPENDED</code> 保证 primary thread 在 resume 前不运行，因此任何规范、实现或证据都不得声称 child 在 <code>ResumeThread</code> 前自读或报告自身 environment。

普通 production launch 依赖同 RuntimeCompositionManifest/profile/capability generation 已通过的 parent serializer/CreateProcess composition proof；每次 launch 仍必须生成独立 parent proof，但不伪称逐 child pre-resume read-back。若未来要求每个真实 target 在任何 target code 前自验，必须由 successor 明确定义 trusted bootstrap/trampoline、其进程替换或 effect protocol，并重新执行 Full Q0。
...
Stop-the-line 按 scope 分流：repo/template policy 或 gate failure 只 suspend 对应 qualification/Authorization；只有 CLI/SDK execution interface、App Server client protocol、managed Worktree isolation、Automations scheduling、permission、Windows sandbox、feature/tool inventory 或 auth adapter 行为可疑时才全局 suspend/incompatible。
...
P4 cohort 在 B2/per-repo scheduling 下运行。B3 只保留为未来 data-only capability contract，不进入 0.2 实施 DAG，也没有 0.2 activation action；P4 全绿后直接进入逐仓 P5。只有 P5 closeout、B2 指标和独立 B3 threat/recovery review 全部具备后，successor capability generation 才可增加 <code>activate_b3_portfolio_generation</code>。这避免为了尚未验证的跨仓便利扩大首发控制面。
...
- SQLite 是唯一 policy authority、transition head 和 mutable scheduler truth；journal 是 append-only effect observation、process/event chronology 和 recovery input，永远不是可自由重放的第二 policy store。Recovery 只接受由当前 DB accepted cursor/head、current fence、sealed segment chain 和 immutable action history共同证明的 event；不得从 journal 中尚未 accepted、已被 supersede、重复或属于旧 generation 的记录创建新 task state、Authorization、capacity lease 或 effect intent。
- 同一 accepted DB/action/event history 在同一 policy generation 下必须推导相同 recovery decision、next legal effects 和 terminal reason；replay fixture 对观察时间、扫描次数和 controller restart count 不敏感。DB cursor 落后时只能按 contiguous sequence CAS 前进；发现 gap、fork、unknown generation 或 journal领先但无法验证 current head 时 park/operator action，不能猜测补状态。
- Cleanup 使用显式 finalizer lane。人工 repair 只能满足、替代或以受控 terminal result关闭 finalizer 条件，不能删除 guard、跳过 read-back、直接标 terminal 或手工清除 journal/DB row 来解除阻塞。
...
- 正常吞吐由 durable terminal/park/recovery 事件触发一次有界的立即 re-evaluation：同一 controller invocation 最多 claim/drain 一个 task，必须先落盘前一 terminal disposition，并由 global capacity=1、queue priority、fence 和 dedup event key 限制；它不常驻循环，也不得从一个 terminal event 扩展为多 writer。
- Windows Task Scheduler 只作为 watchdog/recovery fallback：InteractiveToken、LeastPrivilege/Limited、Hidden、StartWhenAvailable、IgnoreNew、每 5 min 检查 controller liveness、遗失 event/recovery 和只读 status/doctor，ExecutionTimeLimit 12,600 s。它不是正常吞吐时钟；若需补偿 claim，同一次 invocation 仍最多 drain 一个 task。
- Event-driven 或 watchdog invocation 都只允许 <code>batch drain --max-tasks 1 --json</code>、safety recovery 和只读 status/doctor；禁止 login/logout、environment preparation、Authorization activation、ownership repair、cutover/rollback、Native maintenance 和 kill-current。
...
|---|---|---|---|
| Full Q0 | 新 composition；CLI/SDK execution interface、App Server client protocol、managed Worktree isolation、Automations scheduling、sandbox/permission/model-visible inventory、Git/Windows capability、canonicalization、persistence/schema/probe behavior 变化 | staged immutable installation + clean managed probe repos | 产生 Q(C,I,staged identity)，但不自行选择或准入 runtime |
...
- 新或行为变化的 CLI/SDK execution interface、App Server client protocol、managed Worktree isolation、Automations scheduling、provider/runtime engine、sandbox/token/permission 行为、model-visible tool inventory、Git object/ref capability、network/delivery protocol、Windows helper、canonicalization/persistence/schema/migration/probe或 unknown diff：必须 Implementation Acceptance + Full Q0。
...
2. Codex config and capability surface：strict-config、profile、<code>trust_level=untrusted</code>、project config/hooks/rules skip、AGENTS/skills discovery、feature catalog、model-visible tool inventory、ephemeral/history/logging disk diff，以及 Gate/Git <code>--include-managed-config</code> 生效证明。CLI/SDK execution interface、App Server client protocol、managed Worktree isolation 和 Automations scheduling 分别运行同一代的 schema/help/inventory/sandbox/approval/state/recovery probes；一个 surface 的成功不得给另一个 surface 授权，组合使用另做 effect protocol probe。
...
13. CodexPlatformCapabilitySet：每个 candidate surface 的 no-write、no-unowned-effect、state/resume、approval/sandbox、worktree ownership、failure/recovery 和 rollback probes；App Server WebSocket 或 experimental method 不得因能连接而通过，Automations 不得因桌面 UI 存在而被当作稳定脚本化 contract，任一多-surface组合另需组合 effect protocol probe。
...
官方文档定义允许声称的配置语义；pinned 本机 <code>--help</code>、config schema、binary inventory 和行为 probe 决定该 installation 是否兼容。<code>ExecutionInterfaceCapability.v1</code>、<code>ClientProtocolCapability.v1</code>、<code>WorkspaceIsolationCapability.v1</code> 和 <code>SchedulingCapability.v1</code> 分别绑定官方 reference locator/date、surface version、help/schema hashes、tool inventory、sandbox/approval/state/effect probes、config compiler generation 和 known removed/renamed keys；<code>CodexPlatformCapabilitySet.v1</code> 绑定组成 generation 与组合 effect protocol。

任何未知 key、help/schema 与行为不一致、removed feature 复活、model/tool/profile 漂移或外部 effect 变化都禁止动态 fallback。修复必须产生新的 composition/profile/capability generation，并按 Q0TriggerPolicy 执行 Implementation Acceptance、Full/专项 Q0、repo/template qualification、Authorization 和 canary 的确定集合。
...
~~~text
v3.25 narrative candidate
-> P0A normative package closure
-> Baseline Approval
-> P0B Truth Reset
-> P0C Legacy Ownership Guard || P0D no-side-effect package/harness
-> join: P0C guard green + P0D harness green
...
### 16.2 v3.24 successor provenance 与聚焦重验

v3.23 的 <code>NativeThinPathCapabilityEvaluation.v1</code> 已结束，只作为 non-normative predecessor evidence 保留。它证明当时 CLI generation 下 evaluator-owned gates/evidence/recovery 不能被 agent-side prompt gate 替代，但没有晋升 profile，也不证明 v3.25 的 uv/Python/build envelope。v3.25 不重复无关的 18 个 trial；只有产品路由、Native/Batch 边界、model/effort promotion claim 或 execution surface 发生变化时才重跑对应 evaluation cohort。

本 successor 的必跑聚焦重验是 <code>ExactToolchainGateEvaluation.v1</code>：固定 uv/Python/lock/build-constraint/cache generation，在 disposable roots 执行 exact sync、wrong-interpreter、extraneous-package、pytest-plugin、multi-backend-cache、offline/download-deny 和 repeat-build fixtures。结果只可为 <code>exact_gate_contract_preserved</code> 或 <code>supersede_required</code>；任何环境污染未被清除/拒绝、build backend identity 不唯一、child interpreter 不匹配或 clean-root artifact 不一致都阻断 package closure。

本 successor 的额外聚焦重验是 <code>PreResumeEnvironmentProofEvaluation.v1</code>：对 parent serializer 执行 empty/one-entry/case-collision/invalid-key/limit-1/limit/limit+1/double-NUL/flag mismatch fixtures，并由专用 no-write Q0 child 在 resume 后首先报告 environment digest。结果只可为 <code>two_stage_environment_proof_preserved</code> 或 <code>supersede_required</code>；不得把 parent 输入 buffer、controller parser 回读或 fixture simulation 伪装成 child pre-resume observation。

后续如果改变 Batch 禁止面、adapter、authority、并发、Q0 trigger、quality promotion、事实来源、首发 template effect envelope 或本两阶段环境证明，必须冻结 v3.25、创建 v3.26 candidate、lineage、inventory、DAG、selector 和 verifier；不得原地改写 v3.25。

### 16.3 P0A：规范包闭包

- 保留 v3.14-v3.24 精确谱系，归档 v3.24 preapproval inventory/work-item plan；由 <code>BaselineLineage.v4</code> 逐项验证七个不受 successor 语义影响的 carried artifacts。Q0 及后续缺失 artifact 按既有 15 项 inventory 闭包。
...
### 16.5 P0C：Legacy Ownership Guard
...
### 16.6 P0D 与 P1：新 runtime 实现

P0B Truth Reset 后，P0C 与 P0D 可作为两个独立、可回滚分支并行准备。P0D 只建立 <code>runtime/local-ai-runtime</code> package、exact/offline build profile、contracts CLI、empty state root、migration harness 和 no-side-effect doctor；不得读取 legacy DB、注册 repo、创建 ownership、claim、writer、Git object/ref 或 production evidence。P1 的任何实现任务必须等待 P0C guard 与 P0D harness 同时绿色，因此并行只缩短工具链反馈，不削弱双 writer 防线。随后 P1 按可执行 vertical slices 推进：
...
在 sandbox/overlay/CodexPlatformCapabilitySet 完成后，必须运行 <code>adapter_no_write_smoke_v1</code>：只使用 runtime-owned disposable repo、无真实 task、无 writer execution grant，验证 pinned Codex CLI execution-interface framing、effective config、tool inventory、network/path deny 和零 worktree/common-store mutation。其他 surface 各自只执行其被资格化的 no-write probe；它们不因 CLI smoke 自动通过。所有 smoke 都不是 live claim、Q0 或 Implementation Acceptance。
...
- B3 portfolio scheduling 在 0.2 明确 deferred；P4 只验证 B2/per-repo，P5 不等待也不激活 B3。

### 16.8 P5：逐仓 cutover、rollback 与退休
...
P0D 创建新包后，仓库根目录只通过 checked-in <code>new_runtime_exact_v1</code> profile 运行门禁。Environment preparation 与日常验证分开：仅在 lock/toolchain generation 变化或 environment 缺失时，先执行一次可审计的 exact sync；日常 gate 禁止隐式 sync、install 或 download。

~~~powershell
$uv = "<RuntimeToolchainManifest.uv.absolute_path>"
$python = "<RuntimeToolchainManifest.python.absolute_path>"
$constraints = "<RuntimeToolchainManifest.hashed_build_constraints.absolute_path>"

# environment preparation only; not a daily validation gate
& $uv sync --locked --offline --no-python-downloads --python $python --project ./runtime/local-ai-runtime

# fixed validation order
& $uv lock --check --offline --no-python-downloads --python $python --project ./runtime/local-ai-runtime
& $python -I -s -E -m local_ai_runtime toolchain verify-environment --profile new_runtime_exact_v1 --json
& $uv build --offline --no-python-downloads --python $python --build-constraint $constraints --require-hashes --project ./runtime/local-ai-runtime ./runtime/local-ai-runtime
& $uv run --no-sync --offline --no-python-downloads --python $python --project ./runtime/local-ai-runtime python -I -s -E -m pytest
& $uv run --no-sync --offline --no-python-downloads --python $python --project ./runtime/local-ai-runtime python -I -s -E -m local_ai_runtime contracts verify
& $uv run --no-sync --offline --no-python-downloads --python $python --project ./runtime/local-ai-runtime ruff check ./runtime/local-ai-runtime
& $uv run --no-sync --offline --no-python-downloads --python $python --project ./runtime/local-ai-runtime pyright --project ./runtime/local-ai-runtime
python scripts/verify-planning-status.py
python scripts/select-next-work.py
git diff --check
~~~

固定门禁顺序仍是 build -> test -> contract/invariant -> hotspot；最前面的 lock/environment identity 是 supply-chain prerequisite，不伪装成 build。<code>uv lock --check --offline</code> 只验证现有 lock 与 project metadata；<code>verify-environment</code> 必须证明 venv interpreter、installed distribution set、pytest plugin inventory、Ruff/Pyright/build backend 与 manifest/lock/constraints 一致且无 extraneous package。Build report 绑定 backend identity、SOURCE_DATE_EPOCH、member manifest 和 artifact hashes；dist 输出由 checked-in ignore/cleanup policy 管理，并由 verifier 证明未覆盖 tracked file。触及 legacy ownership guard、wire schema 或 compat adapter 时，在新包 test 后补跑：
...
- Ruff、Pyright、pytest 和 contract verifier 必须来自 exact synced environment；build backend 必须来自 manifest-bound hashed build constraints。禁止从 PATH、残留 venv、全局 plugin、uv 自动下载或未绑定 cache 偶然选择工具。
...
- v3.25 本文及全部规范性 schema/catalog/table/example/fixture/verifier 实际落盘；
...
通过阈值为 <code>net_operator_minutes_per_success</code> 至少下降 50%，且 <code>commit-ready successes/operator-hour</code> 提高至少 50%或 P50 admission-to-commit-ready 周期下降至少 30%。Safety、回归/漏检、gate/evidence completeness 或 recovery/rollback 未过时，效率改善无效。P95 wall time、token/成本、冲突/返工与 sample downstream acceptance 必须并列报告，不能只按平均吞吐或 commit-ready 宣布 promotion。
...
Commit-ready 不是长期质量真值。<code>DownstreamOutcomeRecord</code> 是 P4 cohort 和任何 TaskFamily/profile/capability promotion 的抽样输入，绑定 ActiveRuntimeIdentity、task commit、可验证 final integration commit lineage、source identity、观察窗口、human review disposition、merge/reject、后续 CI、revert 和 defect evidence locator。每个 P4 candidate 至少抽样覆盖已接受、被拒绝或返工的 downstream outcome；没有下游反馈必须记为 <code>censored|unknown</code>，不能算 pass，也不能静默从分母删除。runtime 不自动 push、不读取 remote CI、不替人做 merge/reject；操作者只把外部结果作为 secret-safe evidence locator 和 disposition 附回。promotion 不能只按 commit-ready 自循环优化。
...
- [GPT-5.6 Sol migration guidance](https://developers.openai.com/api/docs/guides/upgrading-to-gpt-5p6-sol)
...
- [Codex App Server daemon source](https://github.com/openai/codex/blob/main/codex-rs/app-server-daemon/README.md)
- [Codex Python SDK FAQ](https://github.com/openai/codex/blob/main/sdk/python/docs/faq.md)
- [uv: handling extraneous packages and exact sync](https://docs.astral.sh/uv/concepts/projects/sync/#handling-of-extraneous-packages)
- [uv: reproducible build constraints and hashes](https://docs.astral.sh/uv/concepts/projects/build/#build-constraints)
...
- v3.17 与两份冲突 v3.18 源字节已经归档并双路径验证；v3.21、v3.22、v3.23、v3.24 均保持精确 superseded candidate identity，v3.23/v3.24 package/plan 另存为 non-authoritative frozen history；
- 本文与 v3.25-bound <code>BaselineLineage.v4</code> 落盘后，preapproval inventory 应如实报告 15 required / 9 present / 6 non-present：source、lineage 与七个逐项 carry-forward artifact present，Q0/migration/examples/verifier/manifest/review 尚未闭合；approval inactive、Truth Reset/implementation/P2 均未开始；selector 的唯一 ready work item 应为 <code>LAR-P0A-010 / close_baseline_normative_package_first</code>；
...
本文的决策状态是 <code>candidate / Request changes until v3.25 normative package closure</code>。总体产品与架构方向保持收口；当前下一项是物化 P0A-Q0 的 gate、effective-feature、process/environment 和 resource-limit catalogs，并把本 successor 的两阶段环境证明写成 schema/catalog/fixture/verifier contract，但本切片不运行 live Q0。若后续发现需要改变 Batch 禁止面、adapter、authority、并发、Q0 trigger、quality promotion、事实来源、launch template effect envelope 或两阶段环境证明，必须冻结本候选并创建 v3.26；不得原地改写。只有受控 external operator action 创建的 active BaselineApprovalRecord 存在且未撤销，才允许执行 Truth Reset 和实现；只有 Implementation Acceptance、staged Full Q0、activation pointer CAS 和 immediate preflight 共同形成 ActiveRuntimeIdentity，才允许创建第一个 P2 writer task。
