...
当前 action 是 `close_baseline_normative_package_first`，task 是 `LAR-P0A-007`。`LAR-P0A-006` 已闭合 `ExecutionSafetyContractSet.v1`、EffectPlan、stable writer effect/launch identity、Writer/StageJob suspended atomic Job join、exact stdio handle inheritance/EOF、execution-commit barrier、closed authority union、same-name Job fail-closed 与 non-forking adoption；下一步只物化 event/journal/artifact/receipt/backup contracts。最终 `BaselineManifest.v1.json` 仍不得创建。不得启动 process/Job、持久化 raw output、读取 live auth/sandbox state、运行 live Batch、创建批准记录、Truth Reset、`runtime/local-ai-runtime`、remote push/CI retrieval，或修改 `.ai/config`。
