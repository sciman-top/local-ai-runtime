...
- `WP6`：第一批 cutover drill / fail-closed guard、cutover review / manual approval gate、非破坏 rollback restore drill、archive restore acceptance、operator approval evidence gate、默认未批准的 operator approval template 生成入口、approval hash / sanitized audit 留痕、以及 approval 对 `archive_restore_acceptance_path` 的强引用校验已落地；`--cutover-v2` 在 drill 未 ready 时不修改 `runtime.active_version`，drill ready 后仍默认返回 `manual_approval_required`；`--cutover-rollback-drill-v2` 会验证 v1 DB / v1 runs 源并写出 `archive-restore-acceptance.json`，但不执行 restore；`--confirm-cutover-v2` 还必须绑定通过校验且引用当前 archive restore acceptance 的 `--cutover-approval-ref` 才进入默认入口切换路径；当前已有一条真实 `local_maint` v2 live coding probe completed，eval summary `ok=true`，cutover drill `ready=true / cutover_performed=false`
