...
当前 action 是 `close_baseline_normative_package_first`，task 是 `LAR-P0A-008`。`LAR-P0A-007` 已闭合 `EvidenceContractSet.v1`、event/status field matrix、secret-safe append-only journal、six-condition receipt、no-replace artifact、runtime-managed external evidence、purpose-separated key envelopes 与 anti-rollback backup/restore；下一步只物化 deterministic Git publication contracts。最终 `BaselineManifest.v1.json` 仍不得创建。不得执行 Git publication、remote Git、backup/restore、读取 live auth/sandbox/DPAPI state、运行 live Batch、创建批准记录、Truth Reset、`runtime/local-ai-runtime`、remote push/CI retrieval，或修改 `.ai/config`。
