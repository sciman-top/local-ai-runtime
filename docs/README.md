...
本索引服务于冻结的 `local-ai-runtime-0.2-v3.23` 候选基线。v3.23 是现行 `baseline_candidate`、`blocking_stage=baseline_approval`；正文和 v3.23-bound lineage 已闭合，manifest schema/fixtures/verifier skeleton 已完成但不计为最终 artifact，规范包仍为 `15 required / 2 present / 13 missing`。Native thin-path 的三份 evaluation contract 已封存并绑定 exact identity；在任何 P0A schema/catalog 闭包前，必须执行 `run_native_thin_path_evaluation_first / LAR-P0A-EVAL-002`，不得把合同封存误读为比较或资格化完成。
...
6. [machine work items](D:/CODE/local-ai-dev-orchestrator/docs/plans/local-ai-runtime-0.2-work-items.json)：v3 确定性 DAG，机械约束 package-root/九子包 source layout和 11 项稳定 contract projection，共 65 项；Native thin-path / capability comparative evaluation 的两个前置任务先于 P0A closure，P1A-P1F 有 35 个编码切片，每次只领取 selector 返回的一项。
...
- 当前 candidate 规划已冻结 Native thin-path / capability comparative evaluation contract，下一步才执行固定比较；只有 `preserve_v3_23_semantics` 决定才释放 P0A 规范闭包。P0B Truth Reset 及之后任务全部被 active Baseline Approval 阻断。
