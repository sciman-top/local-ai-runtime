...
当前 package 是 `6/15 present, 9 non-present`。`ProductContract.v2` 与 `QualificationContractSet.v2` 尚未创建，最终 `BaselineManifest.v1.json` 不存在，`runtime/local-ai-runtime` 不存在。当前唯一 machine work item 是 `LAR-P0A-004`；selector action 是 `close_baseline_normative_package_first`。
