...
- `WP6` cutover / archive / 真相面切换：进行中
  - 第一批 `--cutover-drill-v2`、`--cutover-v2` fail-closed guard、cutover review / manual approval gate、`--cutover-rollback-drill-v2` 非破坏恢复路径检查、`--confirm-cutover-v2` 人工确认闸门与 `--cutover-approval-ref` operator approval evidence gate 已落地；真实 `local_maint` v2 live coding probe 已 completed，当前 drill ready 但默认 `--cutover-v2` 仍返回 `manual_approval_required / cutover_performed=false`，缺少 approval evidence 的 confirmed cutover 仍返回 `approval_required / cutover_performed=false`，默认入口仍未切换
