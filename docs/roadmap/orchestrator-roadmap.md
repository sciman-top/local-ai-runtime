...
- `WP6` cutover / archive / 真相面切换：进行中
  - 第一批 `--cutover-drill-v2` 与 `--cutover-v2` fail-closed guard 已落地；真实 `local_maint` v2 live coding probe 已 completed，当前 drill ready 但 `cutover_performed=false`，默认入口仍未切换
