...
- [ ] `K2-T06` run cutover drill and switch the default entrypoint only after gates are green
  - Status note:
    ...
    - 2026-07-08 已补 operator approval evidence gate：`--cutover-v2 --confirm-cutover-v2` 仍必须附带 `--cutover-approval-ref <approval.json>`，且 approval JSON 需校验 schema、`approved_by / approved_at`、当前 review / rollback summary 引用、当前 `archive_restore_acceptance_path` 引用、以及 `default_entrypoint_switch / rollback_restore_required` 风险确认；缺失时返回 `approval_required / cutover_performed=false`
