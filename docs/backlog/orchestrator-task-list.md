...
- [ ] `LAR-P0A-001` ready：关闭 v3.14-v3.20 谱系；归档 v3.17 exact bytes、两份 conflicted v3.18、精确 v3.19 和冻结 v3.20；v3.17 bytes 缺失时 fail closed。
- [ ] `LAR-P0A-002` pending：Narrative/artifact/package 三层版本语义、manifest schema、规范字节和 verifier skeleton；不得创建最终 manifest，`P0A-MANIFEST` 保持 missing。
- [ ] `LAR-P0A-003` pending：Canonical JSON、Git path、Windows identity/collision、alias-aware 8.3 与 `policy_query_denied` probe。
- [ ] `LAR-P0A-004` pending：Product/routing/template、永久 ordinary replay、absent-only secret/admission、原子 resubmission。
...
- [ ] `LAR-P0A-006` pending：Writer/StageJob、marker、JOB_LIST/HANDLE_LIST、精确 stdio/EOF、suspended execution barrier、Authorization/SafetyOnly authority union、fencing/adoption。
...
- [ ] `LAR-P0A-008` pending：Git config/audit、object plan、claim time、no-reflog commit/finalize/ref/remove。
- [ ] `LAR-P0A-009` pending：SubmissionFamily/Task/Attempt/Platform/Repo/Template/Autonomy/Operator policies。
- [ ] `LAR-P0A-010` pending：Q0、feature/process/gate、mandatory write accounting、emergency reserve、optional hard quota 和 resource-limit catalogs。
...
- [!] `LAR-GOV-001` blocked by P0A review and explicit authority：批准或拒绝 v3.21；不得与 review、Truth Reset 或实现合并，AI 不得自签。
...
- [!] `LAR-P1D-002` blocked：attempt environment、durable writer marker、JOB_LIST/HANDLE_LIST suspended launch 与 resume barrier。
...
- [!] `LAR-P1E-003` blocked：attempt-local object plan、deterministic commit、canonical promotion/reachability。
...
- [!] `LAR-P1F-005` blocked：OperatorAction/WorkSession、runbooks、人工分钟计量。
- [!] `LAR-P1F-006` blocked：read-only compat、cutover/rollback dry run、paired/cohort evaluation；不执行 live cutover/cohort。
- [!] `LAR-P1G-001` blocked：全 offline gates、crash/conformance/migration/restore 和 `ImplementationAcceptanceRecord`。

`LAR-P1G-001` 完成后仍不得进入 P2；Full Q0 是独立门。

## Q0 与 P2-P5：有限自治、验证与迁移

- [!] `LAR-Q0-001` blocked：真实 Full Q0 与 P2 Admission；禁止 writer，不能用 legacy probe/simulation 替代。
...
- [!] `LAR-P4-001` blocked：两个真实 repo、30-task cohort、12+ 严格配对效率 case。
