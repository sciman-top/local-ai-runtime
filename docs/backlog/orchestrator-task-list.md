...
- [ ] `F-T01` topology expansion
  ...
  - Status note:
    - 2026-07-08 已补 `remote_non_gui` runner wiring readiness contract 与 acceptance-ref guard：committed `remote_non_gui_probe` 仍保持 `runner_wired=false`，临时测试配置必须同时设置 `runner_wired=true` 与 repo-relative `runner_acceptance_ref` 才会调用注入 runner，runner 失败保持 failed dispatch 且不写成功 result；`F-T01` 未标完成，因为真实 remote host runner acceptance 与真实 GUI-only workload evidence 仍未开始
...
- [ ] `K2-T06` run cutover drill and switch the default entrypoint only after gates are green
  - Status note:
    ...
    - 2026-07-08 已补 `--cutover-rollback-drill-v2`：验证 review summary、恢复 `runtime.active_version = v1` 的配置目标、archive root 可用性、v1 control-plane DB 源、v1 runs 源目录和当前默认入口仍为 v1；该入口写出 `.ai/runs-v2/_cutover/cutover-rollback-drill-summary.json` 与 `.ai/runs-v2/_cutover/archive-restore-acceptance.json`，不执行 restore
    - 2026-07-08 已补 operator approval evidence gate：`--cutover-v2 --confirm-cutover-v2` 仍必须附带 `--cutover-approval-ref <approval.json>`，且 approval JSON 需校验 schema、`approved_by`、UTC `Z` 格式 `approved_at`、当前 review / rollback summary 引用、当前 `archive_restore_acceptance_path` 引用、以及 `default_entrypoint_switch / rollback_restore_required` 风险确认；缺失时返回 `approval_required / cutover_performed=false`
    - 2026-07-08 已补 operator approval template 入口：`--cutover-approval-template-v2` 会生成默认 `approved=false` 的可编辑 approval JSON，并引用当前 review / rollback drill summary 与 archive restore acceptance summary；该入口不执行 cutover，不修改默认入口
    - 2026-07-08 已补 operator approval audit 留痕：approval validation 会记录 `approval_sha256 / approval_byte_count / approved_by / approved_at / archive_restore_acceptance_path`，并写出 sanitized `operator-approval-audit.json`；该留痕不执行 cutover
...
- [x] `P5-T04` remote_non_gui runner wiring readiness
  - Done when:
    ...
    - 临时 `runner_wired=true` 配置必须绑定 repo-relative `runner_acceptance_ref` 后才会调用注入 runner
