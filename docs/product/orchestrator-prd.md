...
- Product baseline：`local-ai-runtime-0.2-v3.25`
...
本文是 v3.25 的产品投影，不补写规范语义。字段、状态、哈希、崩溃窗口和安全边界以候选正文及批准后的 normative package 为准。v3.25 narrative、独立 versioned artifacts 与最终 immutable BaselineManifest 是三个层次；正文和 lineage 完整不代表 package 已闭合。v3.24 candidate/package/plan 是精确 superseded inputs，不能被当作当前实现要求。当前 package 为 `10/15 present, 5 non-present`；七个既有 artifact 只通过 `BaselineLineage.v4` 精确 carry forward，v3.25 新建的 `QualificationGateCatalog.v1` 已 present，下一项是 `LAR-P0A-011 / PersistenceMigrationSpecification.v1 + CrossContractExampleManifest.v1`。
...
v3.25 不把 `uv run --locked` 当作 exact environment 证明。`RuntimeToolchainManifest` 必须绑定 Python 3.11.x patch、解释器 absolute path/file identity/SHA-256、uv absolute path/version/hash、lockfile、installed distributions、pytest plugins、build frontend/backend 和 hashed build constraints。环境准备显式使用 `uv sync --locked --offline --no-python-downloads --python <manifest-python>`；`uv sync` 默认 exact，禁止 `--inexact`，且不存在可执行的 `--exact` 选项；日常 gate 使用 `run --no-sync`，不得隐藏 sync、download 或 PATH fallback。

Windows 环境证明采用两阶段产品契约：每次 launch 都必须先生成 `pre_resume_parent_environment_proof`，证明实际传入 `CreateProcessW` 的 canonical UTF-16 block、digest、limits、terminator 与 flags；Full Q0 再由专用 no-write child 在 resume 后第一个 application action 生成 `post_resume_q0_child_environment_observation` 并比较 digest。普通生产任务只继承同 generation 已通过的 composition qualification，不展示或声称逐 child pre-resume 自读结果。若两者 mismatch，产品状态是 `platform_incompatible`，不是自动修复、降级或继续执行。
...
CLI execution interface 是 0.2 唯一入选的受控 execution surface；SDK、App Server、managed Worktree 和 Automations 仍是独立 capability candidates，不因同一安装包或 predecessor evaluation 自动获得资格。v3.23 Native comparative result 仅作为 non-normative predecessor evidence：不 promotion profile、不删除 evaluator gates，也不驱动当前 selector。任何 v3.25 语义变更必须建立 v3.26 successor。
...
1. **Baseline Approval**：v3.25 正文、15 项 normative package、standalone verifier、preliminary/manifest-closure review 全部绿色并由独立明确授权签发；不要求代码存在。v3.23 predecessor evaluation 不是批准输入；任何 v3.25 语义变化先创建 v3.26。
