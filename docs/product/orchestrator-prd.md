...
- Product baseline：`local-ai-runtime-0.2-v3.23`
...
本文是 v3.23 的产品投影，不补写规范语义。字段、状态、哈希、崩溃窗口和安全边界以候选正文及批准后的 normative package 为准。v3.23 narrative、独立 versioned artifacts 与最终 immutable BaselineManifest 是三个层次；正文和 lineage 完整不代表 package 已闭合。v3.22 是精确 superseded input，不能被当作当前实现要求。
...
### 4.6 Native thin-path 与平台资格化

Baseline Approval 前必须先完成 Native thin-path / capability comparative evaluation。它固定同一 repo snapshot、TaskFamily、模型/effort、tool inventory、sandbox、gates、成功 oracle、重复次数和人工介入分钟定义，对比精简 Codex Native、Native + key gates，以及仅在适用场景下的 Superpowers、Trellis、Hermes。`grill-with-docs` 只可作为低频、高不确定、不可逆决策的人类 ADR/术语对齐路径，不是日常 Batch 前门或自治执行器。

CLI 与 SDK 只构成 execution-interface candidates；App Server 是 client-protocol candidate；managed Worktree 是 workspace-isolation candidate；Automations 是 scheduling candidate。它们分别记录 version/help/schema/tool inventory、sandbox/permission 行为、state/effect boundary、recovery/rollback 和 evidence projection；同一安装包、名称相近或一个 surface 的 Q0 结果都不能推出另一个 surface 已资格化。只有每个组成 capability 和组合 effect protocol 都通过对应 generation、Implementation Acceptance 与 Full Q0，`CodexPlatformCapabilitySet` 才能被 profile 引用。
...
1. **Baseline Approval**：先完成 Native thin-path / capability comparative evaluation，并且只有 `preserve_v3_23_semantics` 才可继续正文和全部 normative artifact 闭合、一致性评审绿色；不要求代码存在。若评测改变规范语义，必须转入 v3.24 successor，不能批准 v3.23。
2. **Implementation Acceptance**：代码、migration、CLI、runbook、legacy conformance、crash、backup/restore 和固定 offline gates 绿色。
