...
本文是 v3.24 的产品投影，不补写规范语义。字段、状态、哈希、崩溃窗口和安全边界以候选正文及批准后的 normative package 为准。v3.24 narrative、独立 versioned artifacts 与最终 immutable BaselineManifest 是三个层次；正文和 lineage 完整不代表 package 已闭合。v3.23 candidate/package/plan 是精确 superseded inputs，不能被当作当前实现要求。当前 package 为 `9/15 present, 6 non-present`；`ProductContract.v2`、`QualificationContractSet.v2` 与 `StatePolicyCatalog.v1` 已闭合，下一项是 `LAR-P0A-010 / QualificationGateCatalog.v1`。
