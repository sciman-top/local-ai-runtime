...
& '$scriptPath' -Action load -EnvFilePath '$envPath' -SkipBackupPrompt | ConvertTo-Json -Compress -Depth 8
"@
...
"@
            ...
            $resultJson = & $runtime.exe -NoProfile -NonInteractive -File $childScriptPath 2>&1
            $exitCode = $LASTEXITCODE
        }
        if ($exitCode -ne 0) {
            $issues += "manage-hermes-provider-session load from .env failed under $($runtime.name) with exit code ${exitCode}: $($resultJson | Out-String)"
        }
        else {
            $rawText = ($resultJson | Out-String).Trim()
            if ([string]::IsNullOrWhiteSpace($rawText)) {
                $issues += "manage-hermes-provider-session produced no parseable output under $($runtime.name)."
                continue
            }
            ...
            try {
                ...
                $issues += "manage-hermes-provider-session did not emit JSON under $($runtime.name): $rawText"
                continue
            }
            $loadedSlots = @($result.loaded_slots)

            if (-not $result.gate_ready) {
                $issues += "manage-hermes-provider-session did not report gate_ready after .env load under $($runtime.name)."
            }

            if ($result.active_slot -ne 'primary') {
                $issues += "Expected active_slot=primary after .env load under $($runtime.name), got '$($result.active_slot)'."
            }

            if ($loadedSlots.Count -ne 2 -or $loadedSlots -notcontains 'primary' -or $loadedSlots -notcontains 'backup') {
                $issues += "Expected loaded_slots to contain primary and backup after .env load under $($runtime.name), got '$($loadedSlots -join ',')'."
            }
        }
    }
}
