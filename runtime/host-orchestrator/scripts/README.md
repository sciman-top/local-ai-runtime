...
- `run-remote-non-gui-promotion.ps1`：运行 deterministic `P5-T03` remote_non_gui promotion evidence suite，产物写到 `private-local/remote-non-gui-promotions/`
- `test-wave1-acceptance.ps1`：串起 `pytest -> contract -> smoke` 的 `W1-T07` repo-side fake-first 验收链

这些脚本只证明 repo-side smoke/simulation/promotion/acceptance 资产已落地，不证明 live `Codex SDK` 真机执行、remote runner、或 live 多 worker scheduler 已验收。
