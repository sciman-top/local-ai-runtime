...
if (-not $RunId) {
    $RunId = 'vm-gui-promotion-' + (Get-Date -Format 'yyyyMMdd-HHmmss')
}
...
try {
    $jsonLines = & uv run python -m host_orchestrator `
        ...
        --vm-gui-promotion-run-id $RunId
    if ($LASTEXITCODE -ne 0) {
        throw "vm_gui promotion runner failed (exit=$LASTEXITCODE)."
    }
}
finally {
    Pop-Location
...
if (-not $summary.ok) {
    ...
    throw "vm_gui promotion suite reported issues."
}
