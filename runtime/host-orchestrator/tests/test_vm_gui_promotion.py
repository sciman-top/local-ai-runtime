...
def test_host_local_runner_hands_off_explicit_vm_gui_profile_before_worker_execution(
    tmp_path: Path,
) -> None:
    from host_orchestrator.canonical_task import write_task

    repo_root = _seed_repo(tmp_path)
    task_id = "TASK-20260707-explicit-vm-gui-profile"
    task_path = repo_root / "tasks" / f"{task_id}.json"
    payload = canonical_task_payload(task_id)
    payload["risk_level"] = "low"
    payload["write_access"] = False
    payload["execution_lane"] = "vm_gui"
    payload["requires_gui"] = True
    payload["worker_profile"] = "vm_gui_probe"
    ...
    class FailIfCalledWorker:
        ...
        def run(self, request: WorkerRequest) -> WorkerResult:
            self.call_count += 1
            raise AssertionError("vm_gui probe profile must hand off before worker execution")
    ...
    runner = HostLocalRunner(
        HostLocalConfig(
            ...
            run_id="explicit-vm-gui-profile-test",
        ),
        worker,
    )
    ...
    assert result_payload["worker_profile"] == "vm_gui_probe"
    assert result_payload["lane"] == "vm_gui"
    assert result_payload["route_reason"] == "repo-owned worker_profile=vm_gui_probe selected from canonical task"
    assert "requires_gui=true" in result_payload["status_reason"]
    assert "host_runtime=host_local" in result_payload["status_reason"]
    assert "selected_lane=vm_gui" in result_payload["status_reason"]
    assert "runner_not_wired" in result_payload["status_reason"]
    assert "worker_profile=vm_gui_probe" in result_payload["status_reason"]
    assert dispatch_payload["worker_profile"] == "vm_gui_probe"
    assert dispatch_payload["execution_lane"] == "vm_gui"
    assert dispatch_payload["route_reason"] == result_payload["route_reason"]


def test_vm_gui_promotion_suite_writes_summary_and_preserves_fail_closed_boundary(
    tmp_path: Path,
) -> None:
    ...
    assert set(summary.worker_lanes.values()) == {"host_local", "vm_gui"}
    assert summary.summary_path.exists()

    outcomes = {outcome.task_id: outcome for outcome in summary.task_outcomes}
    default_outcome = outcomes["TASK-20260707-vm-gui-default-request"]
    ...
    assert "execution_lane=vm_gui" in default_outcome.status_reason
    assert "requires_gui=true" in default_outcome.status_reason
    assert "runner_not_wired" not in default_outcome.status_reason

    promoted_outcome = outcomes["TASK-20260707-vm-gui-profile-promotion"]
    assert promoted_outcome.worker_profile == "vm_gui_probe"
    assert promoted_outcome.final_state == "waiting_handoff"
    assert promoted_outcome.route_reason == "repo-owned worker_profile=vm_gui_probe selected from canonical task"
    assert "requires_gui=true" in promoted_outcome.status_reason
    assert "host_runtime=host_local" in promoted_outcome.status_reason
    assert "selected_lane=vm_gui" in promoted_outcome.status_reason
...
def test_cli_runs_vm_gui_promotion_and_prints_json(
    tmp_path: Path,
    capsys,
) -> None:
    repo_root = _seed_repo(tmp_path)
