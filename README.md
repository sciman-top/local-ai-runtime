...
当前规范候选是 `local-ai-runtime-0.2-v3.23`，状态为 `baseline_candidate`，`blocking_stage=baseline_approval`。冻结正文和 v3.23-bound lineage 已落盘，`BaselineManifest.v1` schema/fixtures/verifier skeleton、`CanonicalizationPolicy.v1` 与 `ProductContract.v1` contract bundles 已完成，但最终 manifest 尚未创建且 standalone verifier 尚未冻结；规范包仍如实为 `15 required / 4 present / 11 missing`，因此仍是 **Request changes**，不是已批准实现基线。
