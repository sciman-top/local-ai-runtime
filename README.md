...
- `P5-T04` 的 `remote_non_gui` runner wiring readiness 已落地；临时测试配置必须同时设置 `runner_wired=true` 与 repo-relative、schema-valid 的 `runner_acceptance_ref` 才能调用注入 runner，runner 失败保持 failed dispatch 且不写成功 `result.json`，但 committed profile 仍未接真实 remote runner
